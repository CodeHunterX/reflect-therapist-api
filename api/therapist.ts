...
const MODEL = 'gpt-3.5-turbo-0125'; // or gpt-4o-mini / gpt-4o
...
async function moderate(content: string) {
  ...
  return data?.results?.[0]?.flagged as boolean;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (typeof user !== 'string' || !Array.isArray(history))
    return res.status(400).json({ error: 'Body must be { user: string, history?: [{role,text}] }' });
...
If user mentions self‑harm -> urge them to call local crisis line (988 in US) and ask if they can stay safe right now. Do not refuse; do not mention you’re an AI model.
    ...
    /* turn chat history into OpenAI format */
    ...history.map((m: any) => ({
